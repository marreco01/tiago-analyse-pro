...
export default function LiveGames() {
  ...
  async function loadLiveGames() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/football/live?limit=12", { cache: "no-store" });
      const data = (await response.json()) as LiveResponse;
      if (!response.ok || !data.success) throw new Error(data.error || "Erro ao carregar jogos ao vivo.");
      setGames(data.games || []);
      setUpdatedAt(data.updatedAt || new Date().toISOString());
      setDemoMode(Boolean(data.demoMode));
      setDemoNotice(data.notice || null);
    } catch (err) {
      ...
      <div className="space-y-4 md:space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-400 md:mt-3 md:text-base">Placar, pressão ofensiva e estatísticas rápidas em tempo real.</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
          <GlassCard className="p-4 md:p-5"><div className="text-xs font-bold text-slate-400 md:text-sm">Jogos live</div><div className="mt-1 text-3xl font-black text-white md:mt-2">{games.length}</div></GlassCard>
        ...
        {!loading && !filteredGames.length ? (
          <GlassCard className="p-10 text-center"><Activity className="mx-auto mb-4 h-10 w-10 text-slate-500" /><h2 className="text-2xl font-black">Nenhum jogo ao vivo encontrado</h2><p className="mt-2 text-slate-400">A API gratuita pode retornar poucos jogos live. Quando a API paga entrar, esta área ficará mais completa.</p></GlassCard>
        ) : null}
