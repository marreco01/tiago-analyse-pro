...
import { getTeamInitials, getTeamLogoCandidates, teams, type Team } from "@/data/teams";
...
import { fetchStatisticsCompare } from "@/lib/statisticsRobot";
import { fetchCornersCompare } from "@/lib/cornersRobot";
import { fetchCardsCompare } from "@/lib/cardsRobot";
import { fetchGoalsCompare } from "@/lib/goalsRobot";
...
export default function Analyze() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      try {
        const stat = await fetchStatisticsCompare(teamA, teamB);
        if (stat.opportunity && data.analysis) {
          data.analysis.bestMarket = stat.opportunity.bestMarket;
          data.analysis.confidence = Math.max(data.analysis.confidence || 0, stat.opportunity.confidence);
          data.analysis.sourceMode = `${data.analysis.sourceMode || "API"} + Robô Estatístico`;
          data.analysis.stats = {
            ...data.analysis.stats,
            over15: stat.opportunity.goals.over15.probability,
            over25: stat.opportunity.goals.over25.probability,
            btts: stat.opportunity.goals.btts.probability,
            averageCorners: stat.opportunity.corners.expected,
            cards: stat.opportunity.cards.expected,
          };
          data.analysis.marketProbabilities = [
            ...(data.analysis.marketProbabilities || []),
            { name: "Escanteios Over 8.5", value: `${stat.opportunity.corners.over85.probability}%`, level: stat.opportunity.corners.over85.level },
            { name: "Escanteios Over 9.5", value: `${stat.opportunity.corners.over95.probability}%`, level: stat.opportunity.corners.over95.level },
            { name: "Cartões Over 3.5", value: `${stat.opportunity.cards.over35.probability}%`, level: stat.opportunity.cards.over35.level },
          ];
        }
      } catch {
        // mantém análise principal mesmo se o robô estatístico não encontrar confronto
      }

      try {
        const corner = await fetchCornersCompare(teamA, teamB);
        if (corner.opportunity && data.analysis) {
          data.analysis.sourceMode = `${data.analysis.sourceMode || "API"} + Robô Escanteios`;
          data.analysis.stats = {
            ...data.analysis.stats,
            averageCorners: corner.opportunity.expectedCorners,
          };
          data.analysis.marketProbabilities = [
            ...(data.analysis.marketProbabilities || []),
            { name: "Escanteios Over 7.5", value: `${corner.opportunity.lines.over75.probability}%`, level: corner.opportunity.lines.over75.level },
            { name: "Escanteios Over 8.5", value: `${corner.opportunity.lines.over85.probability}%`, level: corner.opportunity.lines.over85.level },
            { name: "Escanteios Over 9.5", value: `${corner.opportunity.lines.over95.probability}%`, level: corner.opportunity.lines.over95.level },
            { name: "Escanteios Over 10.5", value: `${corner.opportunity.lines.over105.probability}%`, level: corner.opportunity.lines.over105.level },
          ];
          data.analysis.bestMarket = corner.opportunity.confidence >= (data.analysis.confidence || 0) ? corner.opportunity.bestLine : data.analysis.bestMarket;
          data.analysis.confidence = Math.max(data.analysis.confidence || 0, corner.opportunity.confidence);
        }
      } catch {
        // mantém a análise mesmo sem dados de escanteios
      }

      try {
        const cards = await fetchCardsCompare(teamA, teamB);
        if (cards.opportunity && data.analysis) {
          data.analysis.sourceMode = `${data.analysis.sourceMode || "API"} + Robô Cartões`;
          data.analysis.stats = {
            ...data.analysis.stats,
            cards: cards.opportunity.expectedCards,
          };
          data.analysis.marketProbabilities = [
            ...(data.analysis.marketProbabilities || []),
            { name: "Cartões Over 2.5", value: `${cards.opportunity.lines.over25.probability}%`, level: cards.opportunity.lines.over25.level },
            { name: "Cartões Over 3.5", value: `${cards.opportunity.lines.over35.probability}%`, level: cards.opportunity.lines.over35.level },
            { name: "Cartões Over 4.5", value: `${cards.opportunity.lines.over45.probability}%`, level: cards.opportunity.lines.over45.level },
            { name: "Cartões Over 5.5", value: `${cards.opportunity.lines.over55.probability}%`, level: cards.opportunity.lines.over55.level },
          ];
          data.analysis.bestMarket = cards.opportunity.confidence >= (data.analysis.confidence || 0) ? cards.opportunity.bestLine : data.analysis.bestMarket;
          data.analysis.confidence = Math.max(data.analysis.confidence || 0, cards.opportunity.confidence);
        }
      } catch {
        // mantém a análise mesmo sem dados de cartões
      }

      try {
        const goals = await fetchGoalsCompare(teamA, teamB);
        if (goals.opportunity && data.analysis) {
          data.analysis.sourceMode = `${data.analysis.sourceMode || "API"} + Robô Gols`;
          data.analysis.stats = {
            ...data.analysis.stats,
            over15: goals.opportunity.lines.over15.probability,
            over25: goals.opportunity.lines.over25.probability,
            btts: goals.opportunity.lines.btts.probability,
          };
          data.analysis.marketProbabilities = [
            ...(data.analysis.marketProbabilities || []),
            { name: "Over 0.5 HT", value: `${goals.opportunity.lines.over05HT.probability}%`, level: goals.opportunity.lines.over05HT.level },
            { name: "Over 1.5 gols", value: `${goals.opportunity.lines.over15.probability}%`, level: goals.opportunity.lines.over15.level },
            { name: "Over 2.5 gols", value: `${goals.opportunity.lines.over25.probability}%`, level: goals.opportunity.lines.over25.level },
            { name: "Over 3.5 gols", value: `${goals.opportunity.lines.over35.probability}%`, level: goals.opportunity.lines.over35.level },
            { name: "BTTS", value: `${goals.opportunity.lines.btts.probability}%`, level: goals.opportunity.lines.btts.level },
          ];
          data.analysis.bestMarket = goals.opportunity.confidence >= (data.analysis.confidence || 0) ? goals.opportunity.bestLine : data.analysis.bestMarket;
          data.analysis.confidence = Math.max(data.analysis.confidence || 0, goals.opportunity.confidence);
        }
      } catch {
        // mantém a análise mesmo sem dados de gols
      }
      ...
      try {
        const cup = await fetchWorldCupCompare(teamA, teamB);
        if (cup?.found?.analysis) {
          const analysis = cup.found.analysis;
          const data: ApiResponse = {
            success: true,
            analysis: {
              confidence: analysis.confidence,
              bestMarket: analysis.over15 >= analysis.btts ? "Over 1.5 gols" : "BTTS",
              riskLevel: analysis.confidence >= 75 ? "Baixo" : "Médio",
              summary: `Robô Copa 2026: favorito ${analysis.favorite}. ${analysis.reason}`,
              sourceMode: "Robô Copa 2026",
              stats: {
                over15: analysis.over15,
                over25: analysis.over25,
                btts: analysis.btts,
                averageCorners: analysis.corners,
                cards: 4,
              },
              marketProbabilities: [
                { name: "Over 1.5 gols", value: `${analysis.over15}%`, level: analysis.over15 >= 75 ? "Forte" : "Médio" },
                { name: "Over 2.5 gols", value: `${analysis.over25}%`, level: analysis.over25 >= 65 ? "Forte" : "Médio" },
                { name: "BTTS", value: `${analysis.btts}%`, level: analysis.btts >= 60 ? "Forte" : "Médio" },
                { name: "Escanteios", value: `${analysis.corners}+`, level: "Médio" },
              ],
            },
          };
          setResult(data);
...
function TeamPicker({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  ...
  return (
    <div>
      ...
      {open ? (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/82 p-3 backdrop-blur-md" onClick={() => setOpen(false)}>
          <div
            className="team-picker-modal max-h-[76vh] w-full max-w-[880px] overflow-hidden rounded-2xl border border-yellow-400/35 bg-[#07080c] shadow-[0_0_60px_rgba(250,204,21,0.18)]"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="team-picker-head border-b border-white/10 bg-gradient-to-r from-yellow-400/10 via-orange-500/10 to-red-500/10 p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-yellow-400">ANALYSE PRO 2.0</p>
                  <h3 className="mt-0.5 text-xl font-black text-white">Selecionar time</h3>
                  <p className="text-xs font-semibold text-slate-400">Busque por nome, país ou liga.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="shrink-0 rounded-xl border border-white/10 bg-white/[0.05] px-3 py-2 text-xs font-black text-white hover:border-yellow-400/40"
                >
                  Fechar
                </button>
              </div>
              <div className="team-picker-search mt-3 flex h-10 w-full items-center gap-2 rounded-xl border border-yellow-400/45 bg-black/75 px-3 focus-within:border-yellow-300">
                <Search className="h-4 w-4 shrink-0 text-yellow-400" />
                <input
                  value={query}
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder="Buscar time, seleção ou liga..."
                  className="block h-full min-w-0 flex-1 bg-transparent text-sm font-bold text-white outline-none placeholder:text-slate-500"
                  autoFocus
                />
              </div>
            </div>

            <div className="team-picker-list max-h-[52vh] overflow-auto p-4">
              <button
                type="button"
                onClick={() => {
                  onChange("");
                ...
                className="mb-3 flex w-full items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-left text-sm font-bold text-slate-200 hover:border-yellow-400/30"
              >
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {groupTeams.map((team) => (
                      <button
                        ...
                        className={`team-picker-option flex min-h-[48px] items-center gap-2 rounded-xl border px-3 py-2 text-left transition ${
                          value === team.name
...
function isNationalTeamLogo(team: Team | null) {
  return Boolean(team && (team.logo?.includes("flagcdn.com") || team.league === "Seleção" || team.country === "Copa 2026"));
}
...
function TeamShield({ team }: { team: Team | null }) {
  ...
  return (
    <div className={`flex h-28 shrink-0 items-center justify-center overflow-hidden border border-yellow-400/45 bg-black/75 shadow-[0_0_44px_rgba(250,204,21,0.18)] ${isNationalTeamLogo(team) ? "w-36 rounded-2xl p-2" : "w-28 rounded-[1.4rem] p-4"}`}>
      {team && src ? (
        <img
          key={`${team.name}-${src}`}
          src={src}
...
function SmallShield({ team }: { team: Team | null }) {
  ...
  const candidates = team ? getTeamLogoCandidates(team.name) : [];
  const src = candidates[candidateIndex];
  const isFlag = isNationalTeamLogo(team);

  return (
    <div className={`team-logo-mini flex h-9 shrink-0 items-center justify-center overflow-hidden border border-yellow-400/25 bg-black/70 ${isFlag ? "w-12 rounded-md p-0.5" : "w-9 rounded-lg p-1"}`}>
      {team && src && !failed ? (
        <img
          key={`${team.name}-${src}`}
          src={src}
        ...
        <span className="text-[9px] font-black text-yellow-400">{getTeamInitials(team.name)}</span>
      ) : (
        <LogoMark small />
      )}
    </div>
