...
import { AlertTriangle, RefreshCcw, Search, ShieldAlert, Target, Trophy, Zap } from "lucide-react";
import { PremiumAppShell, GlassCard } from "@/components/PremiumShell";
import { fetchCardsRobot, type CardOpportunity } from "@/lib/cardsRobot";

type Market = "best" | "today" | "over25" | "over35" | "over45" | "live" | "all";
...
export default function CardsRobot() {
  ...
  async function load() {
    setError("");
    try {
      const data = await fetchCardsRobot(market);
      ...
      <div className="space-y-5">
        <GlassCard className="p-6 md:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.25em] text-yellow-300">Robô Cartões</p>
              <h1 className="mt-2 text-4xl font-black text-white md:text-5xl">Mercado de Cartões</h1>
              <p className="mt-3 max-w-3xl text-slate-300">
                Over 2.5, 3.5, 4.5 e 5.5 com atualização automática a cada 1 minuto.
              </p>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <Metric icon={<Target />} label="Over 3.5 forte" value={items.filter((item) => item.lines.over35.probability >= 76).length} />
          <Metric icon={<ShieldAlert />} label="Over 4.5 forte" value={items.filter((item) => item.lines.over45.probability >= 76).length} />
          ...
          {filtered.map((item) => (
            <GlassCard key={item.id} className="p-5">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-4">
                ...
                <Mini label="Média prevista" value={item.expectedCards} />
                <Mini label="Jogo quente" value={`${item.aggressionIndex}%`} />
              </div>
              ...
              {item.liveAlert?.active ? (
                <div className="mt-4 rounded-2xl border border-red-400/25 bg-red-500/10 p-4 text-sm font-black text-red-100">
                  🚨 {item.liveAlert.message} · Próximo cartão: {item.liveAlert.nextCardProbability}%
                </div>
              ) : null}
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-4">
                ...
                <Line line={item.lines.over45} />
                <Line line={item.lines.over55} />
              </div>
            </GlassCard>
          ))}
        </section>
