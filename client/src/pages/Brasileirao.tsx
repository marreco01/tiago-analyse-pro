...
// V45: removido fallbackClubs. Clubes só vêm do Robô Brasileirão Master.
...
export default function Brasileirao() {
  ...
  const [selectedClubA, setSelectedClubA] = useState<ClubOption | null>({ name: "Flamengo" });
  const [selectedClubB, setSelectedClubB] = useState<ClubOption | null>({ name: "Palmeiras" });
  ...
  async function loadBrasileiraoTableRobot() {
    try {
      const response = await fetch("/api/brasileirao/table", { cache: "no-store" });
  ...
  async function loadData() {
    ...
    try {
      ...
      if (!robotData?.standings?.length) {
        ...
        setNextRound([]);
        setRobotInfo(null);
        setError("Robô Brasileirão Master ainda sem snapshot válido.");
        return;
      }
      ...
      setNextRound([]);
      setRobotInfo(null);
      setError("Robô Brasileirão Master indisponível.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <PremiumAppShell>
      {loading ? <AnalysisProcessLoader title="Carregando Brasileirão..." message="Buscando snapshot validado do Robô Brasileirão Master." /> : null}
      ...
      <div className="space-y-5">
          ...
          <div className="relative z-10 grid gap-4 md:gap-7 xl:grid-cols-[1fr_minmax(280px,390px)_260px] xl:items-center">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-wide sm:mt-4 sm:gap-2 sm:text-[11px]">
                ...
                <span className="rounded-full bg-yellow-400 px-3 py-1.5 sm:px-4 sm:py-2 text-black">Master Série A</span>
              </div>
              {nextMatch ? (
        ...
        <GlassCard className="overflow-hidden p-0">
          ...
          {tab === "classificacao" ? (
            ...
            <BrasileiraoAnalysis clubs={clubs} standings={data?.standings || []} selectedA={selectedClubA} selectedB={selectedClubB} onSelect={selectClubForAnalysis} />
          ) : (
...
function friendlyLeagueLabel(value?: string) {
  ...
  if (!text) return "Brasileirão Série A 2026";
  if (/brasileir|serie-a|série a|bra\.1/i.test(text)) return "Brasileirão Série A 2026";
...
function pickBestRoundMatch(matches: Match[], standings: Standing[] = []) {
  const upcoming = matches.filter((match) => !["FT", "AET", "PEN"].includes(match.status));
...
function BrasileiraoAnalysis({ clubs, standings, selectedA, selectedB, onSelect }: { clubs: Club[]; standings: Standing[]; selectedA: ClubOption | null; selectedB: ClubOption | null; onSelect: (club: ClubOption) => void }) {
  const ready = selectedA && selectedB;
  const a = ready ? findClubHistory(selectedA.name, standings) : null;
  const b = ready ? findClubHistory(selectedB.name, standings) : null;
      ...
      <div className="mb-5 rounded-2xl border border-yellow-400/20 bg-yellow-400/[0.04] p-4">
        <div className="mb-3 flex flex-col justify-between gap-3 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap gap-2 text-xs font-black">
            <span className="rounded-xl border border-green-400/25 bg-green-500/10 px-3 py-2 text-green-300">Clube A: {selectedA?.name || "Selecionar"}</span>
            <span className="rounded-xl border border-yellow-400/25 bg-yellow-400/10 px-3 py-2 text-yellow-300">Clube B: {selectedB?.name || "Selecionar"}</span>
          </div>
        </div>
        <div className="grid grid-cols-5 gap-1.5 sm:grid-cols-6 sm:gap-2 md:grid-cols-8 lg:grid-cols-10">
          ...
          }) : <div className="col-span-full rounded-xl border border-white/10 bg-black/30 p-4 text-sm font-bold text-slate-400">Robô Master sem clubes validados no momento.</div>}
        </div>
      </div>
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-emerald-400/20 bg-black/30 p-5">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
              <TeamReport name={selectedA.name} logo={selectedA.logo} history={a} />
              <div className="text-center text-3xl font-black text-yellow-400">VS</div>
              <TeamReport name={selectedB.name} logo={selectedB.logo} history={b} />
            </div>
          ...
          <div className="rounded-2xl border border-yellow-400/25 bg-yellow-400/[0.035] p-5">
            ...
            <ProbabilityBar label={`Vitória ${selectedA.name}`} value={calc.home} />
            <ProbabilityBar label="Empate" value={calc.draw} />
            <ProbabilityBar label={`Vitória ${selectedB.name}`} value={calc.away} />
          </div>

          <div className="grid gap-4 lg:grid-cols-[1.3fr_0.9fr]">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              <p className="mb-4 text-xs font-black uppercase tracking-[0.22em] text-green-300">Comparação completa</p>
              <CompareLine label="Ataque" left={a.attack} right={b.attack} leftName={selectedA.name} rightName={selectedB.name} />
              <CompareLine label="Defesa" left={a.defense} right={b.defense} leftName={selectedA.name} rightName={selectedB.name} />
              <CompareLine label="Forma" left={calc.formA} right={calc.formB} leftName={selectedA.name} rightName={selectedB.name} />
            </div>
            <div className="rounded-2xl border border-yellow-400/25 bg-yellow-400/[0.055] p-5">
              ...
              <p className="mt-4 text-sm font-bold leading-relaxed text-slate-400">Justificativa: {selectedA.name} marcou {a.goalsFor} gols e sofreu {a.goalsAgainst}; {selectedB.name} marcou {b.goalsFor} gols e sofreu {b.goalsAgainst} nos últimos 10 jogos cadastrados.</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/[0.04] p-5">
            ...
            <p className="mt-3 text-sm font-bold leading-relaxed text-slate-300">A análise usa somente o snapshot validado do Robô Brasileirão Master: classificação, gols, saldo, forma recente real/cache e próximos jogos oficiais.</p>
          </div>
        </div>
...
function TeamReport({ name, logo, history }: { name: string; logo?: string; history: TeamHistory }) {
  ...
  return <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4"><div className="flex items-center gap-3"><TeamLogo src={logo} name={name} /><div><p className="text-[10px] font-black uppercase text-slate-500">Clube</p><p className="text-xl font-black text-white">{name}</p></div></div><div className="mt-4 rounded-xl border border-white/10 bg-black/25 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Últimos 10 jogos</p><div className="mt-2 flex gap-1 text-lg">{history.last10.map((r, i) => <span key={i}>{r === "W" ? "🟢" : r === "D" ? "🟡" : "🔴"}</span>)}</div><p className="mt-2 font-black text-slate-200">{wins}V • {draws}E • {losses}D</p><p className="mt-1 text-sm font-bold text-slate-500">{history.goalsFor} gols marcados • {history.goalsAgainst} sofridos</p><p className="mt-3 text-sm font-bold text-slate-400">{history.moment}</p></div></div>;
}
...
function zoneInfo(rank: number) {
  if (rank <= 4) return { label: "Libertadores", row: "border-l-4 border-l-slate-600", badge: "bg-slate-600 text-white" };
  if (rank <= 6) return { label: "Pré-Libertadores", row: "border-l-4 border-l-slate-600", badge: "bg-slate-600 text-white" };
  if (rank <= 12) return { label: "Sul-Americana", row: "border-l-4 border-l-slate-600", badge: "bg-slate-600 text-white" };
...
function StandingsTable({ rows, clubs, topScorers, nextRound, robotInfo }: { rows: Standing[]; clubs: Club[]; topScorers: BrasileiraoTopScorer[]; nextRound: BrasileiraoNextMatch[]; robotInfo?: BrasileiraoTableRobotResponse["robot"] | null }) {
  ...
  return <div className="p-3 md:p-4">
    ...
    <div className="mt-4 flex flex-wrap gap-3 text-xs font-bold"><span className="rounded-full bg-green-500/15 px-3 py-2 text-green-300">🟢 Libertadores</span><span className="rounded-full bg-blue-500/15 px-3 py-2 text-blue-300">🔵 Pré-Libertadores</span><span className="rounded-full bg-yellow-400/15 px-3 py-2 text-yellow-300">🟡 Sul-Americana</span><span className="rounded-full bg-red-500/15 px-3 py-2 text-red-300">🔴 Rebaixamento</span></div>
