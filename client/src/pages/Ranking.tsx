...
type RankingType = "goals" | "over15" | "over25" | "btts" | "corners" | "favorites" | "quality";
...
const rankingCards: Array<{
  ...
  { type: "over15", title: "🔥 Top Over 1.5", desc: "Jogos com maior tendência para sair pelo menos 2 gols.", short: "Over 1.5", icon: Target },
  { type: "over25", title: "🔥 Top Over 2.5", desc: "Confrontos mais fortes para mercados de 3+ gols.", short: "Over 2.5", icon: Goal },
  { type: "btts", title: "🔥 Top Ambas Marcam", desc: "Partidas com maior chance dos dois times marcarem.", short: "BTTS", icon: Trophy },
  { type: "favorites", title: "🔥 Top Favoritos", desc: "Times com favoritismo mais claro nos próximos jogos.", short: "Favoritos", icon: ShieldCheck },
  ...
  { type: "quality", title: "Top Qualidade dos dados", desc: "Confrontos com dados mais confiáveis.", short: "Qualidade", icon: ShieldCheck },
];

function initialType(): RankingType {
  if (typeof window === "undefined") return "goals";
  const type = new URLSearchParams(window.location.search).get("type");
  if (type === "over15" || type === "over25" || type === "btts" || type === "corners" || type === "favorites" || type === "quality" || type === "goals") return type;
  if (type === "both-scored") return "btts";
...
export default function Ranking() {
  ...
  async function loadRanking(type: RankingType, refresh = false) {
    ...
    setError("");
    try {
      const response = await fetch(`/api/football/rankings?type=${encodeURIComponent(type)}&limit=10${refresh ? `&refresh=${Date.now()}` : ""}`, {
        cache: "no-store",
      });
    ...
    <PremiumAppShell>
      {loading ? (
        <AnalysisProcessLoader
          ...
          message="Analisando confrontos e organizando indicadores."
        />
      ) : null}
        ...
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-slate-400">
              Scanner automático dos próximos confrontos: Over 1.5, Over 2.5, Ambas Marcam, Favoritos, Escanteios e Qualidade dos Dados.
            </p>
          </div>
          <button
            type="button"
        ...
        <GlassCard className="overflow-hidden p-5 md:p-7">
          ...
          {error ? (
            ...
            <div className="flex min-h-64 flex-col items-center justify-center text-slate-400">
              ...
              <p className="mt-4 font-bold">Calculando ranking dos próximos confrontos...</p>
            </div>
          ) : items.length ? (
...
function RankingRow({ item, position, type }: { item: RankingItem; position: number; type: RankingType }) {
  ...
  const secondary = type === "over15"
    ? `Over 2.5 ${item.over25Pct || Math.round(item.averageGoals * 18)}% • BTTS ${item.bothScoredPct}%`
    : type === "over25"
      ? `Over 1.5 ${item.over15Pct || Math.round(item.averageGoals * 25)}% • ${item.averageGoals.toFixed(2).replace(".", ",")} gols/jogo`
      : type === "btts"
        ? `${item.averageGoals.toFixed(2).replace(".", ",")} gols/jogo • Over 1.5 ${item.over15Pct || "--"}%`
        : type === "favorites"
          ? `${item.market || `Favorito: ${item.favorite || "--"}`} • risco ${item.quality >= 84 ? "baixo" : item.quality >= 72 ? "médio" : "alto"}`
          : type === "goals"
            ? `${item.bothScoredPct}% gols das equipes`
            : type === "corners"
