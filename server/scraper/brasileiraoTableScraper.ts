...
export type BrasileiraoStandingRow = {
  rank: number;
  team: string;
  short?: string;
  logo?: string;
...
export type BrasileiraoTableCache = {
  updatedAt: string;
  source: string;
  season: number;
  standings: BrasileiraoStandingRow[];
  topScorers: BrasileiraoTopScorer[];
  nextRound: BrasileiraoNextMatch[];
  fixturesUpdatedAt?: string;
};

export type BrasileiraoTopScorer = {
  rank: number;
  player: string;
  team: string;
  goals: number;
...
export type BrasileiraoNextMatch = {
  id: string;
  round: string;
  date: string;
  time: string;
...
export type BrasileiraoTableRobotStatus = {
  id: string;
  name: string;
...
export type BrasileiraoTableRobotLogEntry = {
  id: string;
  createdAt: string;
  robot: string;
...
const ROBOT_ID = "brasileirao-table";
const ROBOT_NAME = "Robô Classificação Brasileirão";
const UPDATE_EVERY_MS = 5 * 60 * 1000;
const FIXTURES_UPDATE_EVERY_MS = 10 * 60 * 1000;

let intervalStarted = false;
let updating: Promise<BrasileiraoTableCache> | null = null;
let cache: BrasileiraoTableCache = {
...
const SNAPSHOT_FILE = join(process.cwd(), "server", "cache", "brasileirao-master-snapshot.json");
...
function saveSnapshotToDisk(next: BrasileiraoTableCache) {
  try {
...
let status: BrasileiraoTableRobotStatus = {
  id: ROBOT_ID,
  name: ROBOT_NAME,
  status: "online",
  visibleToPublic: false,
...
const logs: BrasileiraoTableRobotLogEntry[] = [];
...
const BRASILEIRAO_MASTER_ALIASES: Record<string, string[]> = {
...
function masterClubKey(value: string) {
  ...
  for (const [canonical, aliases] of Object.entries(BRASILEIRAO_MASTER_ALIASES)) {
...
function brasileiraoLogoFor(team: string, current?: string) {
  const key = masterClubKey(team);
  return BRASILEIRAO_LOGOS[key] || BRASILEIRAO_LOGOS[key.replace(/^redbull/, "")] || current;
}
...
function parseEspnScoreboardEvents(data: any): BrasileiraoNextMatch[] {
  ...
  return events.map((event: any, index: number) => {
    ...
    return {
      id: String(event?.id || `brasileirao-next-${yyyyMmDdFromIso(date)}-${index}`),
    ...
    } as BrasileiraoNextMatch;
  }).filter(Boolean) as BrasileiraoNextMatch[];
}

async function fetchEspnNextRound(): Promise<BrasileiraoNextMatch[]> {
  const today = new Date();
  const windows: string[] = [];
  const startLive = new Date(today);
  // Série A: mantém uma janela maior para ao vivo/finalizados não sumirem
  // e para jogo finalizado não parecer que ainda vai começar pelo horário antigo.
  ...
  const urls = windows.flatMap((dates) => [
    `https://site.api.espn.com/apis/site/v2/sports/soccer/bra.1/scoreboard?dates=${dates}&limit=50`,
    `https://site.web.api.espn.com/apis/v2/sports/soccer/bra.1/scoreboard?dates=${dates}&limit=50`,
  ]);
...
function buildNextRoundFromTable(rows: BrasileiraoStandingRow[]): BrasileiraoNextMatch[] {
  // V44: sem grade inventada por posição da tabela. Próxima rodada só vem de fonte pública/cache.
  return [];
}

function buildTopScorersFromTable(rows: BrasileiraoStandingRow[]): BrasileiraoTopScorer[] {
...
function addLog(level: "info" | "success" | "error", message: string) {
  logs.unshift({ id: `brasileirao-${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt: new Date().toISOString(), robot: ROBOT_NAME, level, message });
...
function normalizeRow(entry: any, index: number): BrasileiraoStandingRow | null {
  ...
  return {
    ...
    logo: brasileiraoLogoFor(name, Array.isArray(team?.logos) ? (team.logos.find((logo: any) => String(logo?.rel || "").includes("full"))?.href || team.logos[0]?.href) : team?.logo || undefined),
    points,
    played,
    win: wins,
    draw: draws,
...
function parseEspnStandings(data: any): BrasileiraoStandingRow[] {
  ...
  const rows = candidates
    ...
    .filter((row): row is BrasileiraoStandingRow => Boolean(row?.team));
...
function espnTeamIdFor(row: BrasileiraoStandingRow) {
...
async function fetchTeamRecentFormFromSchedules(row: BrasileiraoStandingRow, season: number) {
  const teamId = espnTeamIdFor(row);
  if (!teamId) return [] as Array<"W" | "D" | "L">;

  const urls = [
    `https://site.api.espn.com/apis/site/v2/sports/soccer/bra.1/teams/${teamId}/schedule?season=${season}`,
    `https://site.web.api.espn.com/apis/v2/sports/soccer/bra.1/teams/${teamId}/schedule?season=${season}`,
    `https://site.api.espn.com/apis/site/v2/sports/soccer/bra.1/teams/${teamId}/schedule`,
  ];
...
async function fetchRealRecentForms(rows: BrasileiraoStandingRow[]) {
  ...
  for (const [start, end] of windows) {
    ...
    const urls = [
      `https://site.api.espn.com/apis/site/v2/sports/soccer/bra.1/scoreboard?dates=${dates}&limit=300`,
      `https://site.web.api.espn.com/apis/v2/sports/soccer/bra.1/scoreboard?dates=${dates}&limit=300`,
    ];
    for (const url of urls) {
      try {
        const json = await fetchJson(url);
...
async function applyRealRecentForms(rows: BrasileiraoStandingRow[]) {
  ...
  const enriched: BrasileiraoStandingRow[] = [];
...
function isTrustedBrasileiraoTable(rows: BrasileiraoStandingRow[]) {
...
function aliasesForClub(team: string) {
  const key = masterClubKey(team);
  const aliases = BRASILEIRAO_MASTER_ALIASES[key] || [];
...
async function fetchTeamRecentFormFromTheSportsDb(row: BrasileiraoStandingRow) {
...
function buildFallbackTable(): BrasileiraoTableCache {
  return {
...
export function getCachedBrasileiraoTable() {
  return cache.standings.length ? cache : buildFallbackTable();
}

export function getBrasileiraoTableRobotLogs() {
  return logs;
}

export function getBrasileiraoTableRobotStatus() {
...
export async function updateBrasileiraoTableRobot(force = false): Promise<BrasileiraoTableCache> {
  const now = Date.now();
  ...
  updating = (async () => {
    ...
    const season = new Date().getFullYear();
    const urls = [
      `https://site.api.espn.com/apis/site/v2/sports/soccer/bra.1/standings?season=${season}`,
      "https://site.api.espn.com/apis/site/v2/sports/soccer/bra.1/standings",
      `https://site.web.api.espn.com/apis/v2/sports/soccer/bra.1/standings?season=${season}`,
    ];
    ...
    for (const url of urls) {
      try {
        const json = await fetchJson(url);
        let standings = parseEspnStandings(json).map((row) => ({ ...row, logo: brasileiraoLogoFor(row.team, row.logo) }));
        if (!isTrustedBrasileiraoTable(standings)) {
          lastError = new Error(`Fonte rejeitada por validação de Série A: ${standings.map((row) => row.team).join(", ")}`);
          continue;
        }
...
export function startBrasileiraoTableRobot() {
  if (intervalStarted) return;
  intervalStarted = true;
  status.status = "online";
  addLog("info", "Robô Brasileirão Master iniciado. Forma recente por múltiplas fontes e sem bolinhas inventadas.");
  void updateBrasileiraoTableRobot(true);
  setInterval(() => void updateBrasileiraoTableRobot(false), UPDATE_EVERY_MS);
}
