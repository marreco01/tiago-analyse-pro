...
import { analyzeWorldCupTeams, loadWorldCupHistory, saveWorldCupHistory } from "./worldcup-history";
import { updateRankingRobot, getRankingRobotItems, type RankingRobotType } from "./scraper/rankingRobotScraper";
import { updateMasterSearchRobot } from "./scraper/masterSearchRobot";
...
type FootballRankingType = "goals" | "over15" | "over25" | "btts" | "both-scored" | "corners" | "favorites" | "quality";
...
function normalizeRankingType(value: unknown): FootballRankingType {
  ...
  if (type === "over15" || type === "over-15") return "over15";
  if (type === "over25" || type === "over-25") return "over25";
  if (type === "both-scored" || type === "btts") return "btts";
  if (type === "corners") return "corners";
  if (type === "favorites" || type === "favoritos") return "favorites";
...
export function registerFootballLive(app: Express) {
  ...
  app.get(["/api/jogos-hoje", "/api/football/today"], async (req: Request, res: Response) => {
    try {
      const master = await updateMasterSearchRobot(Boolean(req.query.refresh));
      const requestedDate = String(req.query.date || todayBrazil());
      ...
      const events = (master.calendar?.events || [])
        .filter((event: any) => event.date === requestedDate)
        .filter((event: any) => includeFinished || event.status !== "finished")
        .slice(0, limit)
        .map((event: any) => ({
          id: event.fixtureId || event.id,
          ...
          competition: event.competition || event.league,
          country: event.country || event.group || "",
          home: event.home,
          away: event.away,
          homeLogo: event.homeLogo,
          awayLogo: event.awayLogo,
          venue: event.venue || "",
          source: "master-search-v26-calendar",
        }));
      res.setHeader("Cache-Control", "no-store, max-age=0");
      res.json({ success: true, date: requestedDate, timezone: "America/Sao_Paulo", finishedIncluded: includeFinished, games: events, source: "master-search-v26-global" });
    } catch (error) {
      sendError(res, error, "Erro ao buscar jogos de hoje pelo Robô Master Global.");
    }
  });
  ...
  app.get("/api/football/upcoming", async (req: Request, res: Response) => {
    try {
      const master = await updateMasterSearchRobot(Boolean(req.query.refresh));
      const limit = Math.max(1, Math.min(100, Number(req.query.limit || 18) || 18));
      const today = todayBrazil();
      const games = (master.calendar?.events || [])
        .filter((event: any) => event.status !== "finished" && event.date >= today)
        .slice(0, limit)
        .map((event: any) => ({
          id: event.fixtureId || event.id,
          fixtureId: event.fixtureId || event.id,
      ...
      res.json({ success: true, games, updatedAt: master.updatedAt, source: "master-search-v26-global" });
    } catch (error) {
      sendError(res, error, "Erro ao buscar próximos jogos pelo Robô Master Global.");
    }
  });
  ...
  app.get("/api/football/live", async (req: Request, res: Response) => {
    ...
    try {
      const master = await updateMasterSearchRobot(Boolean(req.query.refresh));
      const games = (master.live?.games || []).slice(0, limit);
      res.setHeader("Cache-Control", "no-store, max-age=0");
      res.json({
        success: true,
        games,
        demoMode: false,
        updatedAt: master.live?.updatedAt || master.updatedAt,
        source: "master-search-v26-live",
        notice: games.length ? "Ao vivo alimentado pelo Robô Master Global multifontes." : "Nenhuma partida ao vivo encontrada agora nas fontes monitoradas.",
      });
    } catch (error) {
      sendError(res, error, "Erro ao buscar jogos ao vivo pelo Robô Master Global.");
    }
  });
  ...
  app.get("/api/football/brasileirao", async (_req: Request, res: Response) => {
    try {
      ...
      const table = master.brasileiraoTable;
      const dashboard = {
        standings: table.standings || [],
        nextRound: table.nextRound || [],
        topScorers: table.topScorers || [],
      ...
      res.json({
        success: true,
        dashboard,
        source: "master-search-v26-brasileirao",
        cachedForMinutes: 5,
      });
    } catch (error) {
      sendError(res, error, "Erro ao carregar Campeonato Brasileiro Série A 2026 pelo Robô Master Global.");
    }
  });
  ...
  app.get("/api/football/world-cup/history", async (_req: Request, res: Response) => {
    try {
      const database = loadWorldCupHistory();
      res.json({ success: true, database, source: "Banco local WorldCup History" });
    } catch (error) {
      sendError(res, error, "Erro ao carregar histórico das seleções da Copa.");
    }
  });

  app.post("/api/football/world-cup/history/save", async (req: Request, res: Response) => {
    try {
      const database = saveWorldCupHistory(req.body);
      res.json({ success: true, database, source: "Banco local WorldCup History" });
    } catch (error) {
      sendError(res, error, "Erro ao salvar histórico das seleções da Copa.");
    }
  });

  app.get("/api/football/world-cup/history/analyze", async (req: Request, res: Response) => {
    try {
      const home = String(req.query.home || "").trim();
      const away = String(req.query.away || "").trim();
      if (!home || !away) return res.status(400).json({ success: false, error: "Informe home e away." });
      res.json(analyzeWorldCupTeams(home, away));
    } catch (error) {
      sendError(res, error, "Erro ao analisar seleções pelo histórico.");
    }
  });
  ...
  app.get("/api/football/rankings", async (req: Request, res: Response) => {
    try {
      ...
      const limit = Math.max(4, Math.min(10, Number(req.query.limit || 10)));
      const master = await updateMasterSearchRobot(Boolean(req.query.refresh));
      const data = master.rankings || await updateRankingRobot(Boolean(req.query.refresh));
      const items = getRankingRobotItems(type as RankingRobotType, limit);
      res.json({
        success: true,
        type,
        items,
        updatedAt: data.updatedAt,
        source: "Busca Master Global V26 + Robô Rankings",
        cachedForMinutes: 10,
        notice: "Busca Master Global V26: rankings abastecidos pelo calendário, próximos jogos, forma recente e cache central.",
      });
    } catch (error) {
      sendError(res, error, "Erro ao carregar rankings pelo robô público.");
    }
  });
